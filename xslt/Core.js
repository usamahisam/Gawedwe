...
class GaweAppSekoUshamaMustikohisam extends View {
	...
	Menu(params){
		for (let item in params) {
			this.menuParams['__menu__'+item] = params[item];
		}
	}
	__getPageName(){
	...
	openMenu(a){
		this.__onpopstate('__menu__'+a, true);
	}
	close(){
		history.go(-1);
	}
	__viewing(n){
		...
		if (typeof(nb)!=="undefined") {
			...
			for (var item in yb) {
				this.__componentCloseBefore(yb[item]);
			}
			this.__componentCloseBefore(this.regComps[nb]);
		}
		...
		for (var item in l.__he__) {
			this.__componentReady(l.__he__[item]);
		}
		this.__componentReady(l);
		this.regComps[n] = l;
	...
	__clearViewing(n){
		...
		if (typeof(nb)!=="undefined") {
			let yb = this.regComps[nb].__he__;
			for (var item in yb) {
				this.__componentAfterBack(yb[item]);
			}
			this.__componentAfterBack(this.regComps[nb]);
		}

		//
		let y = this.regComps[n].__he__;
		for (var item in y) {
			this.__componentClose(y[item]);
		}
		this.__componentClose(this.regComps[n]);
	...
	__componentClose(l){
		if (typeof(l)!=="undefined") {
			if (typeof(l.onClose)!=="undefined") {
				l.onClose();
			}
		}
	}
	__componentCloseBefore(l){
		if (typeof(l)!=="undefined") {
			if (typeof(l.onCloseBefore)!=="undefined") {
				l.onCloseBefore();
			}
		}
	}
	__componentAfterBack(l){
		if (typeof(l)!=="undefined") {
			if (typeof(l.onAfterBack)!=="undefined") {
				l.onAfterBack();
			}
		}
	}
